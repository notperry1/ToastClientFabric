...
class ComponentSlider(
    ...
    override var settingDef: SettingDef,
    ...
    override fun render(mouseX: Double, mouseY: Double) {
        ...
        TwoDRenderUtils.drawText(
            "${settingDef.minValue}",
            (x + 4).roundToInt(),
            (y + height / 2 + 2).roundToInt(),
            tColor
        )
        TwoDRenderUtils.drawText(
            "${settingDef.maxValue}",
            (x + maxValX).roundToInt(),
            (y + height / 2 + 2).roundToInt(),
            tColor
        )
        ...
        renderUtils.drawRect(
            (x + barX + (((setting.value?.div(
                (settingDef.maxValue
                    ?: return)
            ))?.times(barLength) ?: return))).roundToInt(), barY - 3, 4, 8, Color(255, 255, 255, 255).rgb
        )
    }

    fun isMouseOnKnob(mouseX: Double, mouseY: Double): Boolean {
        val knobX = (x + barX + (((setting.value?.div(
            (settingDef.maxValue
                ?: return false)
        ))?.times(barLength) ?: return false)))
