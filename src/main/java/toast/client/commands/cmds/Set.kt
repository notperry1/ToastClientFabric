    ...
    private fun displaySetting(name: String, setting: Setting, module: Module) {
        when (setting.type) {
            0 -> {
                ...
                (module.settings.getModes(name) ?: return).forEach { mode ->
                    modes.append(mode).append(", ")
                }
                modes = StringBuilder(modes.subSequence(0, modes.length - 3) as String)
                Logger.message(""" Mode: $name, current: ${setting.mode} available: $modes""", Logger.EMPTY, false)
            }
            1 -> Logger.message(""" Value: $name, current: ${setting.value} minimum: ${(module.settings.getSettingDef(name) ?: return).minValue} maximum: ${(module.settings.getSettingDef(name) ?: return).maxValue}""", Logger.EMPTY, false)
            2 -> Logger.message(""" Toggle: $name, state: ${if (setting.enabled ?: return) "enabled" else "disabled"}""", Logger.EMPTY, false)
                ...
                if (args.size > 1) {
                    ...
                    if (setting != null) {
                        if (args.size == 2) {
                            ...
                            when (setting.type) {
                                0 -> {
                                    if (((settingDef ?: return).modes ?: return).contains(args[2])) {
                                        setting.setNewMode(args[2])
                                ...
                                1 -> {
                                    if (NumberUtils.isParsable(args[2])) {
                                        ...
                                        if (newNum <= ((settingDef ?: return).maxValue ?: return)) {
                                            if (newNum >= (settingDef.minValue ?: return)) {
                                                setting.setNewValue(newNum)
                                                Logger.message("""Changed value of setting ${args[1]} to ${args[2]}""", Logger.INFO, false)
                                            } else {
                                ...
                                2 -> {
                                    if (args[2] == "true" || args[2] == "false") {
                                        setting.setNewEnabled(args[2].toBoolean())
                                        Logger.message("""Changed value of setting ${args[1]} to ${args[2]}""", Logger.INFO, false)
                                    } else {
                    ...
                    for ((key, value) in module.settings.settings) {
